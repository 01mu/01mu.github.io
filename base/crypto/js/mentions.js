const Mentions = {
  ...
  <div class="body" v-if="fullVisible">
    ...
    <template v-for="(count, index) in bizCounts">
      <div class="coinpadding flex">
        ...
        <div class="wrapper10">
           <a class="viewposts" :href="'index.html#/posts/symbol/' + place + '/' + count.symbol">
            [View]
          </a>
        </div>
      </div>
  ...
  methods: {
    ...
    init() {
      ...
      if (this.$route.params.place == 'biz') {
        document.title = '/biz/ Mentions'
        this.place = 'biz'
      } else {
        document.title = '/r/CryptoCurrency Mentions'
        this.place = 'reddit'
      }

      this.url = 'https://01mu.bitnamiapp.com/crypto/mentions/' + this.place
    ...
    load() {
      ...
      $.getJSON(this.url + '/' + this.rank, (json) => {
          ctx.formatCounts(json['data'])
          ctx.bizCounts = json['data']
          ctx.showBar = false
